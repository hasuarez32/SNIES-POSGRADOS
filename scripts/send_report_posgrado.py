"""
send_report_posgrado.py

Construcción y envío del correo de reporte SNIES para posgrado por SMTP.

Variables de entorno requeridas:
    SMTP_USER     — dirección de correo remitente
    SMTP_PASS     — contraseña / app-password de la cuenta
    DESTINATARIOS — lista separada por comas (ej. a@uninorte.edu.co,b@uninorte.edu.co)
"""
...
def _bloque_posgrado(res: dict | None) -> str:
    if res is None:
        return (
            "<h3 style='color:#003893;'>Posgrado</h3>"
            "<p style='color:#dc3545;'>"
            "Error procesando Posgrado. Revisar los logs del workflow.</p>"
        )
...
Programas Modificados ({len(modificados)})
</h4>
<p style="font-size:12px;color:#555;font-family:Arial,sans-serif;">
Código SNIES presente en ambos snapshots pero con cambios en:
Modalidad, Créditos, Costo matrícula, Municipio o Nivel de formación.
</p>
...
Reporte Mensual SNIES — Posgrado — {today_str}
</h2>
<p style="font-size:13px;color:#333;">
Monitoreo de programas académicos de posgrado en Colombia.
Los archivos Excel completos van adjuntos a este correo.
</p>
{_bloque_posgrado(resultados.get("posgrado"))}
...
<strong>Generado automáticamente</strong> por el workflow en GitHub Actions.
</p>
</div>
"""
...
def enviar_reporte(resultados: dict, today: date, chart_paths: list | None = None) -> None:
    ...
    msg["Subject"] = f"Reporte SNIES Posgrado — {today.strftime('%d/%m/%Y')}"
    msg["From"] = smtp_user
    ...
    # Adjuntar xlsx de novedades de posgrado
